...
function saveLLDataCRS(monkeyName,expDate,protocolName,folderSourceString,gridType,type)

FsEye=200;
eyeRangeMS{1} = [-150 150]; timePeriodMS{1} = [-500   500]; maxStimPos{1} = 20;
...
%     save([folderExtract '\LL.mat'],'LL','targetInfo','psyInfo','reactInfo');
    save([folderExtract '/LL.mat'],'LL','targetInfo','psyInfo','reactInfo'); % Vinay - for linux
...
%     save([folderExtract '\EyeData.mat'],'eyeData','eyeRangeMS');
    ...
    LL = getStimResultsLLCRS(monkeyName,expDate,protocolName,folderSourceString);
    save([folderExtract '/LL.mat'],'LL');
    [allTrials,goodTrials,stimData,eyeData,eyeRangeMS] = getEyePositionAndBehavioralDataCRS(monkeyName,expDate,protocolName,folderSourceString,eyeRangeMS{type},FsEye);
    save([folderExtract '/BehaviorData.mat'],'allTrials','goodTrials','stimData');
    save([folderExtract '/EyeData.mat'],'eyeData','eyeRangeMS');
    saveEyeDataInDegCRS(monkeyName,expDate,protocolName,folderSourceString,gridType,timePeriodMS{type},FsEye,maxStimPos{type});
end
end
...
contrastPC = [];
temporalFreqHz = [];
spatialPhaseDeg = []; % Vinay - added this
...
%        spatialPhaseDeg = [spatialPhaseDeg [trials.stimDesc.data.spatialPhaseDeg]]; % Vinay - added this
    end
end

% Sort stim properties by stimType
for i=1:4 % [Vinay] - changed from i=1:3 to i=1:4 to read the extra gabor
...
%        LL.(['spatialPhaseDeg' num2str(i-1)]) = spatialPhaseDeg(gaborIndexFromStimulusOn{i}); % Vinay - added this
%         LL.(['radiusDeg' num2str(i-1)]) =
%         radiusDeg(gaborIndexFromStimulusOn{i}); % Vinay - added this. Now
%         commented because it's already included in an 'if' loop above. 
    end
else
...
for i=1:numTrials
    ...
    if isfield(trials,'trialEnd')
        ...
        if (allTrials.eotCodes(trialEndIndex)==0) &&  (allTrials.certifiedNonInstruction(trialEndIndex)==1) ...
                && (allTrials.catchTrials(trialEndIndex)==1) % Work on only Correct Trials, which are not instruction, catch or uncertified trials
            % [Vinay]: changed allTrials.catchTrials(trialEndIndex)==0 to
            % allTrials.catchTrials(trialEndIndex)==1 so that catch trials
            % are included. This is required for the test recordings where
            % all trials are set to catch trials
...
folderSave = [folderName 'segmentedData/eyeData/']; % Vinay - for linux
...
[eyeDataDegX,eyeDataDegY] = convertEyeDataToDeg(eyeData(useTheseStims));
...
%%%%%%%%-------------===============^^^^^^^===========---------------%%%%%%
% [Vinay]
% CRS related functions

function [LL] = getStimResultsLLCRS(monkeyName,expDate,protocolName,folderSourceString)
...
function [allTrials,goodTrials,stimData,eyeData,eyeRangeMS] = getEyePositionAndBehavioralDataCRS(monkeyName,expDate,protocolName,folderSourceString,eyeRangeMS,Fs)
...
function saveEyeDataInDegCRS(monkeyName,expDate,protocolName,folderSourceString,gridType,timePeriodMS,FsEye,maxStimPos)
...
% More data saved for GRF protocol
[eyeXAllPos,eyeYAllPos,xs] = getEyeDataStimPosCRS(monkeyName,expDate,protocolName,folderSourceString,timePeriodMS,FsEye,maxStimPos);
save([folderSave 'EyeDataStimPos.mat'],'eyeXAllPos','eyeYAllPos','xs','timePeriodMS');
end
function [eyeXAllPos,eyeYAllPos,xs,timePeriodMS] = getEyeDataStimPosCRS(monkeyName,expDate,protocolName,folderSourceString,timePeriodMS,Fs,maxStimPos)
