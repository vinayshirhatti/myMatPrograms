% writeMPfiles takes the following inputs:
...
% signal to be taken for gabor analysis. X(:,L1:L2) is taken 
% for analysis. If not specified, the entire signal is taken for analysis.
...
fn1 = [fn 'ImportData_SIG/']; % Vinay - for linux
makeDirectory(fn1)

if N > 1    % More than one channel
	for Chnum = 1:N
        	if Chnum <= 10
            		fidw = fopen([fn1,'sig.dat.00',num2str(Chnum-1)],'w');
        	elseif Chnum<=100
            		fidw = fopen([fn1,'sig.dat.0',num2str(Chnum-1)],'w');
        	else
            		fidw = fopen([fn1,'sig.dat.',num2str(Chnum-1)],'w');
        	end
