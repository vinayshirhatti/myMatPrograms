...
% Vinay - added this loop ahead to set the frameRate as per the monitor
benq = 1;
if benq
    frameRate = 100;
else
...
dllName = 'I:\programs\requiredResources\nsNEVLibrary64.dll'; % vinay
...
% Vinay - made a loop to check for CRS
if strncmpi(protocolName,'GRF',3)
    goodStimTimes = extractDigitalDataGRF(digitalEvents,digitalTimeStamps,folderExtract,1, frameRate); % Vinay - added frameRate
elseif strncmpi(protocolName,'CRS',3)
    goodStimTimes = extractDigitalDataCRS(digitalEvents,digitalTimeStamps,folderExtract,1, frameRate); % Vinay - added frameRate
end
...
getLFP=1;getSpikes=0; % Vinay - set getSpikes to 0 for EEG recordings
