...
disp(['Total number of Analog channels recorded: ' num2str(cAnalog) ', electrodes: ' num2str(electrodeCount) ', aInp: ' num2str(ainpCount)]);
...
if getLFP && (cAnalog>0)
    ...
    if electrodeCount ~= 0 % Vinay - only if there are any electrodes recorded
    ...
    if electrodeCount ~= 0 % Vinay - only if there are any electrodes recorded
        if cElectrodeListIDsStored > 0
            for i=1:cElectrodeListIDsStored
                ...
                %save([outputFolder 'elec' num2str(electrodesStored(i))],'analogDataFull','analogInfo'); % save it
                ...
                %save([outputFolder 'elec' num2str(electrodesStored(i))],'analogDataFullNotched','analogInfo'); % save it
                ...
                save([outputFolder 'elec' num2str(electrodesStored(i))],'analogData', 'analogDataFull', 'analogDataFullNotched', 'analogDataNotched','analogInfo');
    ...
    if ainpCount>0
        for i=1:ainpCount
            ...
            %save([outputFolder 'ainp'
            %num2str(analogInputNums(i))],'analogData','analogInfo'); %
            %Vinay - commented above
            ...
            %save([outputFolder 'elec' num2str(analogInputNums(i))],'analogDataFullNotched','analogInfo'); % save it
    ...
    if electrodeCount == 0 % Vinay - assign 0 to electrodesStored if electrodeCount is 0, else it remains uninitialized
        electrodesStored = 0;
    end
...
if electrodeCount ~= 0 % Vinay - Do spike decomposition and segmentation only if any electrode is recorded
    if getSpikes
